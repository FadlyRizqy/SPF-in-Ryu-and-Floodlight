...
class ProjectController(app_manager.RyuApp):
    ...
    def get_paths(self, src, dst):
        ...
        paths = []
        queue = [(0, src, [])]  # Prioritas antrian menggunakan jarak terpendek
        visited = set()
                ...
                if node == dst:
                    ...
                    for next_node in self.adjacency[node]:
                        if next_node not in visited:
                            next_cost = cost + self.get_link_cost(node, next_node)
                            heappush(queue, (next_cost, next_node, path))
    ...
    def get_optimal_paths(self, src, dst):
        ...
        return sorted(paths, key=lambda x: self.get_path_cost(x))[:paths_count]
